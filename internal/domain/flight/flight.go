...
func (f *Flight) AssignSeat(passengerId PassengerPassportID, seatNumber inventory.SeatNumber) (*Ticket, error) {
	untilDeparture := f.ScheduledDeparture.Sub(time.Now())
	if untilDeparture < f.CloseBookingBuffer {
		return nil, ErrBookingClosed
	}

	assignment := f.Seats[seatNumber]
	if assignment.PassengerID != "" {
	...
	ticket := NewTicket(f.ID, passengerId, seatNumber, assignment.Price)
...
var (
	...
	ErrNegativeBuffer    = errors.New("negative close booking buffer")
