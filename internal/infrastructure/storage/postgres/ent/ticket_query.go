...
func (_q *TicketQuery) QueryFlight() *FlightQuery {
...
func (_q *TicketQuery) WithFlight(opts ...func(*FlightQuery)) *TicketQuery {
	query := (&FlightClient{config: _q.config}).Query()
...
//
//	client.Ticket.Query().
//		GroupBy(ticket.FieldPassengerID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Ticket.Query().
//		Select(ticket.FieldPassengerID).
//		Scan(ctx, &v)
...
func (_q *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	if query := _q.withFlight; query != nil {
		if err := _q.loadFlight(ctx, query, nodes, nil,
			func(n *Ticket, e *Flight) { n.Edges.Flight = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TicketQuery) loadFlight(ctx context.Context, query *FlightQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Flight)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Ticket)
	for i := range nodes {
		if nodes[i].flight_tickets == nil {
			continue
		}
		fk := *nodes[i].flight_tickets
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "flight_tickets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
